...
function CredUpdateOTP() {
    ...
    useEffect(() => {
        if (!orgContext.showOTPModal) return;
    ...
    async function handleResendOTP() {
        try {
            await orgContext.profileUpdateHandler({ orgContext, alertContext })
    ...
    async function otpSubmitHandler() {
        try {
            ...
            const req = new Request(`/api/org/update/verify`, {
                method: "POST",
                headers: {
                    "Content-Type": "application/json",
                    auth: localStorage.getItem("auth")
                },
                body: JSON.stringify({ otp, ...orgContext.profileEditData })
            });
...
function OrgProfile() {
    ...
    const { name, email, phone, credits } = orgContext.profile;
    const { newEmail, newPhone } = orgContext.profileEditData;
    ...
    function onChangeHandler(ev) {
        const { name, value } = ev.target;
        orgContext.setProfileEditData({ ...orgContext.profileEditData, [name]: value });
    }

    function handleProfileUpdate() {
        orgContext.profileUpdateHandler({ setFormErrors, alertContext, orgContext });
    }
        ...
        <div className="grow font-medium px-3 flex flex-col overflow-hidden md:flex-row md:items-center lg:px-0 md:gap-10">
            ...
            <div className="grow space-y-2 2xl:space-y-5">
                ...
                <div className="flex flex-col gap-y-1 md:flex-row md:items-center">
                    ...
                    <p className="grow p-3 rounded-md bg-gray-100 border-1 border-gray-300">{name}</p>
                </div>
                                ...
                                <div className="grow grid grid-cols-1">
                                    ...
                                    <div
                                        ...
                                        onClick={() => {
                                            ...
                                            orgContext.setProfileEditData({ newPhone: '', newEmail: email })
                                        }}
                                    >
                                ...
                                <div className="grow grid grid-cols-1">
                                    ...
                                    <div
                                        ...
                                        onClick={() => {
                                            ...
                                            orgContext.setProfileEditData({ newEmail: '', newPhone: phone })
                                        }}
                                    >
                                        <i className="fa-solid fa-pen-to-square fa-lg"></i>
                                    </div>
